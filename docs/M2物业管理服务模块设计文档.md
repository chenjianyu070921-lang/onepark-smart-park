...
| 实际端口 | workorder **HTTP 8091 / gRPC 9091**、visitor **HTTP 8092**、parking **HTTP 8093**、notice **HTTP 8094** |
...
### 1. `workorder-service`（工单服务）HTTP `:8091` / gRPC `:9091`
...
### 2. `visitor-service`（访客服务）HTTP `:8092`
...
### 3. `parking-service`（停车服务）HTTP `:8093`
...
### 4. `notice-service`（公告通知服务）HTTP `:8094`
...
**状态流转**（待派单(0)→处理中(1)→待验收(2)→已完成(3)；异常：待验收→驳回→处理中；取消/关闭：任意活跃态→已关闭(4)），与第八章确认 FSM 保持一致。
...
WorkOrder {
    ...
    Status        int8        // 工单状态码: 0待派单 1处理中 2待验收 3已完成 4已关闭(终态), 与 internal/state/fsm.go 一致
    Priority      int8
    Location      string
...
VisitorRecord {
    ...
    // 多方式核验字段(P2): 对齐 deploy/sql/m2_p2_visitor_verify.sql
    IdNo          string     // 身份证号(加密存储, 核验方式=3; 当前明文, 加密待组长确认)
    VerifyChannel int8       // 核验方式 1二维码 2手机号 3身份证 4人脸 5工牌
    FaceToken     string     // 人脸特征令牌(核验方式=4)
    BadgeNo       string     // 工牌号(核验方式=5)
    CreatedAt time.Time
}

// 访客黑名单(P2): 人员维度禁入名单, 邀请/签入实时拦截; 对齐 deploy/sql/m2_p2_visitor_blocklist.sql
VisitorBlocklist {
    ID int64; TenantID int64
    VisitorName string
    Phone string              // 手机号(拉黑维度之一)
    IdNo  string              // 身份证号(拉黑维度之一, 加密存储; 当前明文)
    Reason string             // 拉黑原因
    EffectiveFrom, EffectiveTo *time.Time  // 生效时间窗(NULL=立即/永久)
    Status int8               // 1生效(禁入) 2解除(允许通行)
    OperatorID int64; CreatedAt, UpdatedAt time.Time
}
```
...
Notice {
    ...
    // 撤回审计字段(P2): 对齐 deploy/sql/m2_p2_notice_recall.sql
...
### 1. 工单状态机
```
建单(create, 仅记流水): 即 待派单(0)
待派单(0) --(assign)--> 处理中(1) --(submit)--> 待验收(2) --(approve)--> 已完成(3)
待验收(2) --(reject)--> 处理中(1)                       // 驳回回到处理中
待派单(0)/处理中(1)/待验收(2)/已完成(3) --(close)--> 已关闭(4)   // 任意活跃态可关闭
说明: 已完成(3) 为正常终态; 已关闭(4) 为终态(无出边). reject 仅从 待验收 触发, 回到 处理中.
```
...
flowchart TB
    ...
    subgraph M2["M2 物业管理"]
        workorder["workorder-service 8091/9091"]
        visitor["visitor-service 8092"]
        parking["parking-service 8093"]
        notice["notice-service 8094"]
    end
    subgraph M3["M3 园区安防"]
...
5. **端口/配置**：用 `deploy/.env.example` 的 `${VAR}`，勿硬编码 8091 等端口。
