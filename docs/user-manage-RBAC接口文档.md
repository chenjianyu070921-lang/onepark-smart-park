...
**创建用户** `POST /api/users`
```json
请求: {"username":"zhangsan","password":"Secret@123","nickname":"张三","status":1}
...
| 7 | POST | `/api/users/roles` | 用户-角色分配（幂等） |
...
| # | 方法 | 路径 | 说明 |
|---|------|------|------|
| 8 | POST | `/api/menus` | 创建菜单（含 permission） |
| 9 | POST | `/api/roles/menus` | 角色-菜单（权限）分配（幂等） |
...
| # | 方法 | 路径 | 说明 |
|---|------|------|------|
| 10 | POST | `/api/permissions/check` | 校验用户是否拥有某 permission |
...
见 `app/user-manage/internal/logic/rbac_crud_test.go`（`TestRBACCRUD`），覆盖上述 10 个接口的成功与异常路径：
