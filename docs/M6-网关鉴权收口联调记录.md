...
| 任务 | 优先级 | 状态 | 说明 |
|------|--------|------|------|
| JwtSecret 启动强校验 | P0 | ✅ 已完成（已修正） | 空密钥**无论何种环境一律 `fatal` 拒绝启动（fail-closed）**；去除原 `validateJwtSecret` 中 dev/test「仅告警放行」的误导性分支 |
| 统一对外网关鉴权收口 | P0 | ✅ 已完成 | 网关按 `Mode` 强制启用鉴权；auth-service 改「只透传」；下游 9 服务已统一 `IdentityFromHeader` |
| RBAC 五表 SQL + CRUD 自测 | P1 | ✅ 已完成 | 五表 SQL 已存在（`sys.sql`+`rbac_data_scope.sql`）；补**接口文档** + **Go 单测**（10 接口成功/异常路径） |
...
| 文件 | 变更 |
|------|------|
| `app/auth-service/auth.go` | 启动强校验改为 **fail-closed**：空密钥无论环境一律 `fatal`，去除 dev 仅告警的放行分支（与 `svc.NewServiceContext` 的空密钥 `panic` 守卫语义统一，消除矛盾的「dev 放行」死代码）；`server.Use(middleware.JwtAuth(ctx))` → `server.Use(middleware.IdentityFromHeader)`（下游只透传） |
...
| `app/user-manage/internal/logic/rbac_crud_test.go` | **新增** `TestRBACCRUD`：10 接口 CRUD 自测（DSN 门控，无 DB 自动 Skip） |
| `docs/user-manage-RBAC接口文档.md` | **新增**：10 接口契约 + 错误码 + 自测说明 |
