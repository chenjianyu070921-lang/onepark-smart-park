...
func (m *sysUserModel) Insert(ctx context.Context, u *SysUser) error {
	// 写入时按当前租户归属: 未显式指定则取网关注入的 x-tenant-id, 保证新建用户落入正确园区.
	if u.TenantId == 0 {
		u.TenantId = ctxdata.GetTenantId(ctx)
	}
...
func (m *sysUserModel) FindByID(ctx context.Context, id uint64) (*SysUser, error) {
	var u SysUser
	if err := m.db.WithContext(ctx).Scopes(datascope.FromCtx(ctx)).Where("id = ? AND deleted_at IS NULL", id).First(&u).Error; err != nil {
		return nil, err
	}
	return &u, nil
}
...
func (m *sysUserModel) FindByUsername(ctx context.Context, username string) (*SysUser, error) {
	var u SysUser
	if err := m.db.WithContext(ctx).Scopes(datascope.FromCtx(ctx)).Where("username = ? AND deleted_at IS NULL", username).First(&u).Error; err != nil {
		return nil, err
	}
	return &u, nil
}
...
func (m *sysUserModel) Update(ctx context.Context, u *SysUser) error {
	return m.db.WithContext(ctx).Scopes(datascope.FromCtx(ctx)).Model(&SysUser{}).
...
func (m *sysUserModel) FindList(ctx context.Context, page, size int, keyword string) ([]*SysUser, int64, error) {
	var list []*SysUser
	var total int64
	tx := m.db.WithContext(ctx).Model(&SysUser{}).Scopes(datascope.FromCtx(ctx)).Where("deleted_at IS NULL")
	if keyword != "" {
